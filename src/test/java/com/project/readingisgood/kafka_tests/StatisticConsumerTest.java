...
import com.project.readingisgood.consumer.StatisticConsumer;
...
class StatisticConsumerTest {
    ...
    @SpyBean
    private StatisticConsumer statisticConsumer;
    ...
    void order_consume_from_kafka_test() throws JsonProcessingException, InterruptedException {
        ...
        producer.send(new ProducerRecord<>("statistic-topic", 0, String.valueOf(order.getId()), message));
        producer.flush();

        verify(statisticConsumer, timeout(5000).times(1))
                .statisticTopicListener(orderArgumentCaptor.capture());
